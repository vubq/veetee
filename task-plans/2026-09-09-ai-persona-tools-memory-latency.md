...
- Follow-up 2026-09-10 (load + corpus + cert): `scripts/load_probe.py` đo text-turn đồng thời — 1×5 và 2×10 success 100% (p50 ~7.7s full-turn), 4×5 success 90% (2 LLM first-token timeout, TTS queue-wait tới ~6s); artifact `benchmark-artifacts/load-probe-20260910.json`. Corpus seed 42→72 (30 template `dev`, held-out giữ nguyên). Cert 100-attempt đang chạy nền. Tests 163.
