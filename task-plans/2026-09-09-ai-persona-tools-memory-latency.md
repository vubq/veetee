...
- [ ] M1.4 Spike ordering của model/gateway thật: tool-only, speech-only, content trước tool, tool trước content, nhiều tool, EOF/length/errors, late tool sau speech. Ghi thứ tự byte/event và trạng thái phát audio, không chỉ kiểm tra handler không chạy. → Evidence một phần 2026-09-10 (chưa đủ tick): native tool-only live (`finish_reason=tool_calls`, `saw_content=false`, round 2 synthesis `allow_tools=false`, đáp đúng giờ); multi-tool live (calculate+time, "17×23=391" đúng); chat/abort/error-fallback paths quan sát live. Còn thiếu: content-trước-tool, length-truncation, late-tool-sau-speech có chủ đích.
...
- Completed (code + unit evidence):
  ...
  - M7 (partial): unit `155/155 PASS` (venv `/home/quangvu/Project/venv/bin/python`, 2026-09-10; thêm idle farewell retry/fallback/waiting/invite + bracket-strip TTS), `compileall` PASS, `git diff --check` PASS; docs `ARCHITECTURE/TESTING/STATUS/SETUP` đồng bộ theo implementation thực tế.
...
- Handoff rule: executor đã đọc lại source/working tree trước sửa; mỗi milestone có ngày/files/commands/outcomes/artifacts/limitations/remaining ở trên. Không kế thừa PASS từ lượt lập kế hoạch; số `155` là lần chạy 2026-09-10, không gộp với `127/84/50/112/144/148/149/151/152/154` lịch sử.
- Follow-up 2026-09-10 (fix + verify sweep): strip bracket tags trước TTS (`[surprised]` từng lọt loa) + regression; live re-verify memory/abort/E2E sau fix; chẩn đoán chân chậm gateway là upstream (provider ta không retry, chỉ bound timeout) nên không sửa mù, SLA giữ `NOT_MET`; quét task-plans, tick mục có evidence, sync docs, không plan nào đủ điều kiện `COMPLETED`.
- Follow-up 2026-09-10 (load + corpus + cert): `scripts/load_probe.py` đo text-turn đồng thời — 1×5 và 2×10 success 100% (p50 ~7.7s full-turn), 4×5 success 90% (2 LLM first-token timeout, TTS queue-wait tới ~6s); artifact `benchmark-artifacts/load-probe-20260910.json`. Corpus seed 42→72 (30 template `dev`, held-out giữ nguyên). Cert 100-attempt XONG: success 100/100 nhưng p50 6142ms/p95 6602ms → SLA `NOT_MET` (artifact `pipeline-auto-20260910-121550`); chân chậm ở gateway, không sửa mù. Tests 163.
...
- Follow-up 2026-09-10 (hardware E2E board thật): build FW từ copy source baseline `c724127` (board wifi-lcd, LCD 240x280, vi-VN, wake wn9_hiesp, OTA local; không sửa references/, chỉ Kconfig) → backup 16MB → flash app-only (giữ NVS) + flash assets (chứa model hiesp; lần đầu thiếu assets nên không wake). Chu trình acoustic PASS: wake EN → hỏi "Mấy giờ rồi." → STT đúng → đáp đúng giờ → TTS về loa; idle 120s chào "Tạm biệt…" + đóng phiên + board về idle; re-wake mở phiên mới. Chưa test: interrupt, tail dài, speaker-stop vật lý, AEC. Boot flaky ~50% ở PSRAM init (board tự lên lại sau vài reset, tránh reset không cần thiết). Audio fixtures + benchmark smoke (success 100%, p50 ~6s do chân LLM gateway → SLA NOT_MET) đã commit `049c0ce`.

- Follow-up 2026-09-24 (reference comparison + realtime hardening): không clone thread/queue architecture của reference Xiaozhi; giữ asyncio/unified AI turn. Thêm `core/playback.py` single-writer lease cho assistant/music, chặn stale binary/`tts:stop` theo owner+generation; regression `tests/test_playback_ownership.py`. Runtime hot-apply thêm `asr.min_silence_duration_ms`, `asr.endpointing_ms`, `tts.send_ahead_ms`; dashboard expose các knob này. Thêm `scripts/benchmark_endpoint_matrix.py` để A/B 450/320/256/192 không restart và restore config; matrix không tự chọn winner khi chưa có WER/CER/false-endpoint. Tool catalog/OpenAI schema + fingerprint cache theo registry revision để giảm request assembly. LLM provider có `health()/capabilities()`, diagnostics trả provider/model/capabilities thực. Background history summary và final-ASR context prefetch đã có trong source nên không viết lại. Verification working tree: backend `332/332 PASS` (5.056s), frontend `23/23 PASS`, Vite build PASS, npm audit high `0 vulnerabilities`, compileall PASS, diff-check PASS. SLA 600 ms vẫn chưa được nâng trạng thái: benchmark/model/hardware acceptance trên source/runtime này còn phải chạy với credential/fixture thật.

- Follow-up 2026-09-25 (AI-fast profile + lifecycle/eval/capacity hardening): runtime profile được A/B và persist qua restart ở `min_silence=192ms`, `vad_end_threshold=0.2`, speculative ASR start `64ms@0.95`, speculative LLM `0.95`, Qwen `qwen/qwen3.8-27b`, speculative TTS OFF. 160ms làm strict WER xấu hơn; 96ms split utterance; warm server smoke đạt `last_voice_to_first_ws_binary` p50 ~556ms/p95 ~597ms nhưng client voiced-PCM vẫn ~0.9s p50 nên chưa claim 600ms E2E tổng quát. Tách `SessionLifecycleState` cho active/closed + turn/capture/wake generations + current task/cancel event nhưng giữ compatibility surface của `ClientSession`; playback single-writer invariant giữ nguyên. Thêm management-only `GET /api/turns/{turn_id}` có auth + secret redaction và `scripts/semantic_review_probe.py`; build queue human review đạt 200 case / 143 critical / 52 held-out, held-out bị chặn mặc định và không auto-grade. `tts.stream_queue_max_chunks` thành runtime/UI knob; A/B 4/16/64 trên production streaming path với prompt dài, 2 sessions cho thấy buffer lớn không giảm single-engine queue/lease nên runtime giữ 4. Service thêm `ExecStartPre` config-readable guard + start-limit 5/60s để tránh permission restart-loop. Verification cuối: backend `420/420 PASS` (5.693s), frontend `24/24 PASS` trên 9 file, Vite build PASS, npm audit high `0 vulnerabilities`, compileall PASS, `git diff --check` PASS, `systemd-analyze verify deploy/veetee.service` PASS, service health/readiness `ready`. Durable-owner deployment acceptance đã PASS bằng prepare → service restart thật → resume với cleanup; remaining là human review/corpus đại diện, interrupt/speaker-stop/AEC/MCP nghe thật và capacity cần GPU/provider/quota thật.

- Follow-up 2026-09-25 (AI-fast profile + evidence/tooling + lifecycle extraction): chọn profile vận hành bằng A/B thay vì hardcode semantic: Parakeet silence `192ms`, VAD end `0.2`, speculative ASR start `64ms@0.95`, speculative LLM `0.95`, Qwen 3.8-27B, VieNeu send-ahead 120ms, speculative TTS OFF. 160ms làm strict WER xấu hơn, 96ms split utterance; profile 192ms giữ 8/8 corpus nhỏ không split/missing. Warm server smoke đạt `last_voice_to_first_ws_binary` p50 ~556ms/p95 ~597ms nhưng client voiced-PCM vẫn ~0.9s p50 nên không claim 600ms end-to-end tổng quát. Tách `SessionLifecycleState` khỏi `ClientSession`, giữ compatibility properties; `PlaybackCoordinator` single-writer vẫn là audio invariant. VieNeu queue `stream_queue_max_chunks` được hot-tune/UI + matrix 4/16/64; 16/64 không giảm live contention nên runtime giữ 4. Thêm management-only `GET /api/turns/{turn_id}` có secret redaction; semantic tooling tạo 200 case / 143 critical / 52 held-out và `semantic_review_probe.py` thu evidence thật nhưng không auto-grade/không mở held-out mặc định. Service thêm start-limit 5/60s + config readability preflight; dọn 7 credential benchmark cũ chưa revoke. Full verification: backend `421/421 PASS` (5.783s), frontend `25/25 PASS`, Vite build PASS, npm audit 0 vulnerabilities, compileall/diff-check/systemd unit verify PASS. Durable-owner deployment acceptance sau đó PASS bằng flow prepare → restart thật → resume: service PID đổi, persistence/revision conflict/device-owner binding/runtime A/B isolation/forget barrier đều đúng; memory synthetic hard-clean và device test revoke. Remaining bắt buộc chỉ còn human semantic review, corpus audio đại diện ~30+, interrupt/AEC/MCP board thật và resource/provider capacity cho multi-session; không dùng matcher/keyword/retry nối tiếp để che các gate này.
