...
- Diagnostics P0: audio debugger metrics-only có thời hạn, Manager self-test có
  confirmation và health/system snapshot bounded, không lưu raw audio hoặc sửa
  Wi-Fi/NVS.

### Đầu ra

- “Đặt âm lượng 55”, “đổi biểu cảm”, “lấy device status” chạy end-to-end.
- Manager có tab Chẩn đoán với health, self-test và audio metrics polling; khi
  thiết bị chưa mở voice session, UI hiển thị hướng dẫn đánh thức thiết bị.

### Gate

Tool call unauthorized/timeout/malformed được từ chối an toàn.
Diagnostics host contract/firmware build phải pass; phần mic/loa/LCD và live MCP
trên board được nghiệm thu riêng, không suy ra từ host test.
