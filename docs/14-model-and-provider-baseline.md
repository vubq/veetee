...
### 2.2.2 Codex subscription trong 9Router
...
- Có thể dùng `cx/gpt-5.4-mini` hoặc model `cx/*` cho prototype cá nhân/LAN sau khi
...
Model `cx/gpt-5.4-mini` đã pass smoke tool call và chấp nhận
`reasoning_effort=none|low`; cần benchmark tiếng Việt, latency, quota reset và hành
vi khi quota/upstream hết trước khi chọn làm model production. Với voice, ưu tiên
model không phải `*-review`, đặt `reasoning_effort=none` cho lượt bình thường và chỉ
nâng lên `low` theo agent policy khi request cần suy luận thêm. Reasoning event phải
được giữ cho planner/telemetry có kiểm soát nhưng tuyệt đối không đưa
...
### 2.2.4 Topology Veetee V1 đã chốt
