...
psql -d veetee -f migrations/003_device_lifecycle_ota.sql
```

Rollback only a local/test database:

```bash
psql -d veetee -f migrations/003_device_lifecycle_ota.down.sql
...
Control-plane integration tests must use a separate database named exactly `veetee_test`
through `VEETEE_TEST_DATABASE_DSN`. Tests fail closed instead
