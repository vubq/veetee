...
def _planner_system_prompt(
    ...
    *,
    ...
    return (
        ...
        "silently noop it. If the transcript requests an operation represented by any "
        "available tool, plan.action MUST be call_tool_then_respond (or "
        "execute_pending_tool) and plan.tool_call MUST contain that exact catalog name "
        "and schema-valid arguments; never answer as if the operation succeeded without "
        f"a tool result. {response_text_instruction}"
...
def _response_system_prompt(profile: SessionProfile, tools: ToolBroker) -> str:
    ...
    return (
        ...
        "For a streaming tool, follow plan.tool_response_phase: before means one brief "
        "anticipatory notice that the requested stream is about to start, without claiming "
        "success; after means report the supplied result truthfully. When an after result "
        "says the stream fully played or completed, say it has finished and naturally ask "
        "whether the user wants another selection. Generate wording from the published "
        "locale and personality; do not use a fixed phrase. "
