...
export const deviceHealthSchema = z.object({
  ...
  network: z.object({
    ...
    websocketReconnectAttemptCount: z.number().int().nonnegative().default(0),
    websocketReconnectExhaustedCount: z.number().int().nonnegative().default(0),
    lastDisconnectReason: z.number().int().nonnegative(),
  }),
  audio: z.object({
    captureTaskRunning: z.boolean(),
    playbackTaskRunning: z.boolean(),
    transportUplinkQueueDrops: z.number().int().nonnegative().default(0),
    transportUplinkQueueHighWater: z.number().int().min(0).max(1024).default(0),
...
export const wakeProfileSchema = z.object({
  ...
  sendWakeAudio: z.boolean().default(false),
  activation: detectorProfileSchema,
  interrupt: detectorProfileSchema.nullable(),
  version: z.number().int().positive(),
  publishedVersion: z.number().int().nonnegative(),
