...
class ConversationLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_contextual_goodbye_closes_with_default_semantic_lifecycle(self):
        ...
        self.assertTrue(session.config.intent.semantic_end_enabled)
        self.assertTrue(session.config.conversation.end_intent_ai_enabled)
    ...
    async def test_normal_chat_uses_semantic_lifecycle_but_stays_open_on_continue(self):
        phrase = "Mấy giờ rồi?"
        llm = CountingLLM()
